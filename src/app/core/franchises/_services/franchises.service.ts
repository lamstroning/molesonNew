...
const API_URL = '/api/user/franchises/';
...
export class FranchisesService {
  ...
  constructor(private http: HttpClient, private tokenService: TokenService) {
    this.getFranchises('all').subscribe(res => {
      ...
      //   this.franchisesList.push(new FranchisesModel(item));
      // }

    });
  }

  buyFranchise(_id: string, stocks: number): Observable<any> {
    return this.http.post<any>(API_URL + 'pay', {_id, stocks}, {headers: this.tokenService.getUserTokenHeader()});
  }

  getFranchisesById(id: string, key: string): Observable<any> {
    return this.http.post<any>(API_URL + 'get', {key}, {headers: this.tokenService.getUserTokenHeader()}).pipe(
      concatMap(next => {
        if (next.status === 'success') {
          return from(next.data).pipe(filter((item: any) => item._id === id));
        } else {
          return null;
        }
      })
  ...
  getFranchises(key: string): Observable<any> {
    return this.http.post<any>(API_URL + 'get', {key}, {headers: this.tokenService.getUserTokenHeader()}).pipe(
      concatMap(next => {
        if (next.status === 'success') {
          return from(next.data).pipe(filter((f: any) => !f.isHidden));
        } else {
          return null;
        }
      })
  ...
  filterFranchises() {
    ...
    this.franchisesList = this.franchisesList.filter(item => {
      ...
      return item.status.title === this.filterType;
    });
    this.franchisesList = this.franchisesList.filter(item => {
      if (!this.filterName) {
        return item;
